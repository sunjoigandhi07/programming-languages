...
evalMathExpr = do 
    ...
    let r = evalListOfExprs env exprs in 
        case calcMathList op r of 
            Right v -> return v 
            Left err -> evalError err 
...
evalCompExpr = do
    (env, CompExpr op exprOne exprTwo) <- next 
    let resultOne = getValue (eval evalExpr (env, exprOne))
        resultTwo = getValue (eval evalExpr (env, exprTwo)) in 
            case calcCompExpr op resultOne resultTwo of 
                Right v -> return v 
                Left err -> evalError err 
...
evalNotExpr = do
    ...
    case getValue $ eval evalExpr (env, expr) of 
        -- check if the result is left or right 
        Right (BoolVal val) -> return $ BoolVal $ not val 
